...
int loadPhonebook(contact **phonebook, int *size) { //Загрузка данных из файла
    int fd = open("filebook", O_RDONLY);    //Открытие файла filebook только для чтения
    if (fd == -1) { //Если файловый дискриптор равен -1, значит файла не было при запуске программы
        return 0;   //Выход из функции
    }
    struct stat st; //Получение информации о файле
    if (fstat(fd, &st) == -1){
        printf("fstat error\n");
        close (fd);
        return -1;
    }
    off_t fileSize = st.st_size; //Определение размера файла
    int contactsCount = fileSize / sizeof(contact); //Определение количества структур contact в файле
    if (contactsCount <= 0) {   //Если они отсутствуют -
        close(fd);              //Закрыть файловый дискриптор
        return 0;               //Выход из функции
    }
    contact *temp = realloc(*phonebook, contactsCount * sizeof(contact));   //Перевыделение памяти для определённого количества контактов
    if (temp == NULL) { //Проверка что память была успешно выделена
        printf("Memory reallocation error\n");
        close(fd);
        return -1;
    }
    *phonebook = temp;      //Обновление указателя
    *size = contactsCount;  //Обновление размера структуры (Переменная использутеся во внутренней логике программы)
    for (int i = 0; i < contactsCount; i++){    //Чтение данных из файла блоками
        int bytesRead = read(fd, &((*phonebook)[i]), sizeof(contact));
        if (bytesRead != sizeof(contact)){  //Проверка что чтение блока прошло успешно
...
int savePhonebook(contact *phonebook, int size) {   //Запись данных в файл
    //Открытие файла filebook только для записи, при необходимости он будет создан.
    //Также содержимое будет полностью перезаписано
    int fd = open("filebook", O_WRONLY | O_CREAT | O_TRUNC, 0644);  //0644 - задание прав доступа к файлу
    if (fd == -1) { //Проверка что файл был успешно открыт
        printf("Error opening file for writing\n");
        return -1;
    }
    for (int i = 0; i < size; i++){
        int bytesWritten = write(fd, &phonebook[i], sizeof(contact));   //Запись данных в файл блоками
        if (bytesWritten != sizeof(contact)){   //Проверка что запись успешна
